(() => {
    ...
    function renderCharts(id) {
        ...
        if (chartTriwulan) chartTriwulan.destroy();
        if (chartAkhir) chartAkhir.destroy();

        const triwulanChartOptions = {
            chart: { type: 'bar', height: 350 },
            series: [
                { name: 'Target', data: data.triwulan_target },
                { name: 'Realisasi', data: data.triwulan_realisasi }
            ],
            xaxis: { categories: ['Triwulan 1', 'Triwulan 2', 'Triwulan 3'] },
            title: { text: 'Target vs Realisasi per Triwulan' }
        };
        ...
        const akhirChartOptions = {
            ...
            title: { text: 'Perbandingan Target & Realisasi Akhir' },
